...
const CACHE_NAME = `grownet-${CACHE_VERSION}`;
const RUNTIME_CACHE = `grownet-runtime-${CACHE_VERSION}`;
const API_CACHE = `grownet-api-${CACHE_VERSION}`;
const OFFLINE_PAGE = '/grownet-offline.html';
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(
        keys.filter((k) => (k.startsWith('grownet-runtime-') || k.startsWith('grownet-api-')) && !k.includes(CACHE_VERSION))
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.pathname.startsWith('/api/') || url.pathname.startsWith('/grownet/api/')) {
    event.respondWith(
...
self.addEventListener('message', (event) => {
  ...
  if (event.data?.type === 'CLEAR_CACHE') {
    caches.keys().then((keys) => Promise.all(keys.filter((k) => k.startsWith('grownet-')).map((k) => caches.delete(k))));
  }
});
