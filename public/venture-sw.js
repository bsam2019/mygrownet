...
const CACHE_NAME = `venture-${CACHE_VERSION}`;
const RUNTIME_CACHE = `venture-runtime-${CACHE_VERSION}`;
const API_CACHE = `venture-api-${CACHE_VERSION}`;
const OFFLINE_PAGE = '/venture-offline.html';
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(
        keys.filter((k) => (k.startsWith('venture-runtime-') || k.startsWith('venture-api-')) && !k.includes(CACHE_VERSION))
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.pathname.startsWith('/api/') || url.pathname.startsWith('/venture/api/')) {
    event.respondWith(
...
self.addEventListener('message', (event) => {
  ...
  if (event.data?.type === 'CLEAR_CACHE') {
    caches.keys().then((keys) => Promise.all(keys.filter((k) => k.startsWith('venture-')).map((k) => caches.delete(k))));
  }
});
