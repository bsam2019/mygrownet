...
const CACHE_NAME = `bizdocs-${CACHE_VERSION}`;
const RUNTIME_CACHE = `bizdocs-runtime-${CACHE_VERSION}`;
const API_CACHE = `bizdocs-api-${CACHE_VERSION}`;
const OFFLINE_PAGE = '/bizdocs-offline.html';
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(
        keys.filter((k) => (k.startsWith('bizdocs-runtime-') || k.startsWith('bizdocs-api-')) && !k.includes(CACHE_VERSION))
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.pathname.startsWith('/api/') || url.pathname.startsWith('/bizdocs/api/')) {
    event.respondWith(
...
self.addEventListener('message', (event) => {
  ...
  if (event.data?.type === 'CLEAR_CACHE') {
    caches.keys().then((keys) => Promise.all(keys.filter((k) => k.startsWith('bizdocs-')).map((k) => caches.delete(k))));
  }
});
