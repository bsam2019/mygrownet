...
const CACHE_NAME = `growstorage-${CACHE_VERSION}`;
const RUNTIME_CACHE = `growstorage-runtime-${CACHE_VERSION}`;
const API_CACHE = `growstorage-api-${CACHE_VERSION}`;
const OFFLINE_PAGE = '/growstorage-offline.html';
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(
        keys.filter((k) => (k.startsWith('growstorage-runtime-') || k.startsWith('growstorage-api-')) && !k.includes(CACHE_VERSION))
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.pathname.startsWith('/api/') || url.pathname.startsWith('/growstorage/api/')) {
    event.respondWith(
...
self.addEventListener('message', (event) => {
  ...
  if (event.data?.type === 'CLEAR_CACHE') {
    caches.keys().then((keys) => Promise.all(keys.filter((k) => k.startsWith('growstorage-')).map((k) => caches.delete(k))));
  }
});
