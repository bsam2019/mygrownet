...
abstract class AbstractPaymentGateway implements PaymentGatewayInterface
{
     ...
     */
    protected function makeRequest(string $method, string $url, array|string $data = [], array $headers = []): array
    {
        try {
            $http = Http::withHeaders($headers);
            if (is_string($data)) {
                $http = $http->withBody($data, $headers['Content-Type'] ?? 'application/xml');
            }
