...
class PdfGeneratorService
{
    ...
    /**
     * Create the PDF instance using only the 5 working Blade templates
     */
    private function createPdf(Document $document)
    {
        ...
        // Only use the 5 working Blade templates
        $validTemplates = ['classic', 'modern', 'minimal', 'professional', 'bold'];
        if (!in_array($template, $validTemplates)) {
            \Log::warning('Invalid template, falling back to classic', ['template' => $template]);
            $template = 'classic';
        }
    ...
    private function getTemplateName(Document $document): string
    {
        ...
        // Return template as-is (can be old template, advanced template, or custom template)
        return $template;
    }
