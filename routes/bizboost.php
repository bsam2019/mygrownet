...
|
| - Main domain: mygrownet.com/bizboost/*   → served under /bizboost prefix
| - Subdomain:   bizboost.mygrownet.com/*   → served at root (no prefix)
|
*/
...
// ============================================================
// 1. MAIN DOMAIN ROUTES (mygrownet.com/bizboost/*)
// ============================================================
...
Route::domain('bizboost.mygrownet.com')->name('bizboost.sub.test')->get('/test-subdomain-route', function () {
    Log::info('BizBoost test route HIT', ['host' => request()->getHost(), 'url' => request()->fullUrl(), 'route' => request()->route()?->getName()]);
    return response('bizboost test OK: ' . request()->getHost());
})->withoutMiddleware([\App\Http\Middleware\DetectSubdomain::class]);
...
Route::domain('bizboost.mygrownet.com')->group(function () use ($registerBizBoostAuthRoutes) {
    ...
    Route::middleware(\App\Http\Middleware\BizBoostStandalone::class)->group(function () {
        ...
        Route::get('/', [WelcomeController::class, 'index'])->name('bizboost.sub.welcome');

        // Public mini-website
        Route::prefix('biz')->name('bizboost.sub.public.')->group(function () {
            Route::get('/{slug}', [BusinessController::class, 'publicPage'])->name('business');
            Route::get('/{slug}/products', [BusinessController::class, 'publicProducts'])->name('products');
            Route::get('/{slug}/product/{productId}', [BusinessController::class, 'publicProduct'])->name('product');
            Route::post('/{slug}/contact', [BusinessController::class, 'publicContact'])->name('contact');
        ...
        // Public Marketplace
        Route::prefix('marketplace')->name('bizboost.sub.marketplace.public.')->group(function () {
            Route::get('/', [App\Http\Controllers\BizBoost\MarketplaceController::class, 'publicIndex'])->name('index');
            Route::get('/category/{category}', [App\Http\Controllers\BizBoost\MarketplaceController::class, 'publicCategory'])->name('category');
            Route::get('/search', [App\Http\Controllers\BizBoost\MarketplaceController::class, 'publicSearch'])->name('search');
        });
