...
# Fix Production Investor Messages Status Column Issue
# This script ensures the investor_messages table has the required status column
...
php artisan migrate:status | grep "investor_messages"
...
try {
    \$count = DB::table('investor_messages')
        ->where('investor_account_id', 1)
        ->where('direction', 'to_investor')
        ->where('status', 'unread')
        ->count();
    echo \"✅ Query successful. Unread messages: \$count\n\";
} catch (Exception \$e) {
