...
const existingImageUrl = props.category.image ? growmartImage(props.category.image) : null;
...
const submit = () => {
    form.put(route('admin.growmart.categories.update', props.category.id));
};
</script>

<template>
    <Head title="Edit Category - GrowMart Admin" />
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 max-w-2xl">
            <form @submit.prevent="submit" class="space-y-4">
                ...
                <div>
                    ...
                    <div v-else-if="existingImageUrl && !imagePreview" class="mb-2 relative inline-block">
                        <img :src="existingImageUrl" class="w-32 h-32 object-cover rounded-lg border border-gray-200" />
                    </div>
                    ...
                    <p class="text-xs text-gray-400 mt-1">Supports JPEG, PNG, WebP. Max 2MB. Uploading a new file replaces the existing image.</p>
                </div>
