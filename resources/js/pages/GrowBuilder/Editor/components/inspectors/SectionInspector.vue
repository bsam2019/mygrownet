    ...
    <div class="flex flex-col h-full min-h-0">
        ...
        <div class="flex border-b border-gray-200 flex-shrink-0">
            <button
                ...
                :class="['flex-1 py-1.5 text-xs font-medium transition-colors border-b-2', activeTab === 'content' ? 'text-blue-600 border-blue-600' : 'text-gray-500 border-transparent hover:text-gray-700']"
            >
                Content
            </button>
            <button
                @click="emit('update:activeTab', 'style')"
                :class="['flex-1 py-1.5 text-xs font-medium transition-colors border-b-2', activeTab === 'style' ? 'text-blue-600 border-blue-600' : 'text-gray-500 border-transparent hover:text-gray-700']"
            >
                Style
            </button>
            <button
                @click="emit('update:activeTab', 'advanced')"
                :class="['flex-1 py-1.5 text-xs font-medium transition-colors border-b-2', activeTab === 'advanced' ? 'text-blue-600 border-blue-600' : 'text-gray-500 border-transparent hover:text-gray-700']"
            >
                Advanced
            </button>
        </div>

        <!-- Compact Inspector Content -->
        <div :class="['flex-1 overflow-y-auto p-2', darkMode ? 'custom-scrollbar-dark' : 'custom-scrollbar']">
            ...
            <div v-if="activeTab === 'content'" class="space-y-3">
                ...
                <template v-if="section.type === 'hero'">
                    <div>
                        ...
                        <input
                            ...
                            class="w-full px-2 py-1.5 border border-gray-300 rounded-md text-xs text-gray-900"
                        />
                    </div>
                    <div>
                    ...
                    <div class="border-t border-gray-200 pt-3">
                        <label class="block text-xs font-medium text-gray-700 mb-1.5">Background Type</label>
                        <div class="flex gap-1.5 mb-2">
                            <button
                                @click="updateContent('backgroundType', 'image')"
                                :class="[
                                    'flex-1 flex items-center justify-center gap-1.5 px-2 py-1.5 rounded-md border-2 transition-colors text-xs',
                                    (!section.content.backgroundType || section.content.backgroundType === 'image')
                                        ? 'border-blue-500 bg-blue-50 text-blue-700'
                                        : 'border-gray-200 text-gray-600 hover:border-gray-300'
                                ]"
                            >
                                <PhotoIcon class="w-3.5 h-3.5" aria-hidden="true" />
                                Image
                            </button>
                            <button
                            ...
                            >
                                <FilmIcon class="w-3.5 h-3.5" aria-hidden="true" />
                                Video
                            </button>
                        </div>
                    </div>
                    ...
                    <div v-if="!section.content.backgroundType || section.content.backgroundType === 'image'">
                        ...
                        <div v-if="section.content.backgroundImage" class="relative mb-1.5">
                            <img :src="section.content.backgroundImage" class="w-full h-20 object-cover rounded-md" />
                            <button @click="updateContent('backgroundImage', '')" class="absolute top-1 right-1 p-0.5 bg-red-500 text-white rounded-full hover:bg-red-600">
                                <XMarkIcon class="w-3 h-3" aria-hidden="true" />
                            </button>
                        </div>
                        <button
                            @click="emit('openMediaLibrary', section.id, 'backgroundImage')"
                            class="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 border-2 border-dashed border-gray-300 rounded-md text-gray-500 hover:border-blue-400 hover:text-blue-600 transition-colors"
                        >
                            <PhotoIcon class="w-4 h-4" aria-hidden="true" />
                            <span class="text-xs">{{ section.content.backgroundImage ? 'Change' : 'Add Background' }}</span>
                        </button>
                    </div>
                    
                    <!-- Video Background -->
                    <div v-else-if="section.content.backgroundType === 'video'">
                        <label class="block text-xs font-medium text-gray-700 mb-1">Video Background</label>
                        <div v-if="section.content.videoBackground" class="relative mb-1.5">
                            <img 
                            ...
                            <div class="absolute inset-0 flex items-center justify-center bg-black/30 rounded-md">
                                <div class="w-8 h-8 bg-white/90 rounded-full flex items-center justify-center">
                                    <FilmIcon class="w-4 h-4 text-gray-900" />
                                </div>
                            </div>
                            <button @click="updateContent('videoBackground', ''); updateContent('videoThumbnail', '')" class="absolute top-1 right-1 p-0.5 bg-red-500 text-white rounded-full hover:bg-red-600">
                                <XMarkIcon class="w-3 h-3" aria-hidden="true" />
                            </button>
                        </div>
                        <button
                        ...
                        >
                            ...
                            <span class="text-xs">{{ section.content.videoBackground ? 'Change Video' : 'Add Video' }}</span>
                        </button>
                        <p class="text-[10px] text-gray-500 mt-1">YouTube or Vimeo videos work best</p>
                    </div>
                    
                    <!-- Overlay Settings (for image/video backgrounds) -->
                    <div v-if="section.content.backgroundImage || section.content.videoBackground" class="border-t border-gray-200 pt-3">
                        <label class="block text-xs font-medium text-gray-700 mb-1.5">Overlay</label>
                        <div class="space-y-2">
                            <div>
                                <label class="block text-[10px] text-gray-500 mb-1">Overlay Color</label>
                                <div class="flex gap-1.5">
                                    <button
                                        @click="updateContent('overlayColor', 'black')"
                                        :class="[
                                            'flex-1 py-1.5 rounded-md border-2 text-xs font-medium transition-colors',
                            ...
                            <div v-if="section.content.overlayColor === 'gradient'" class="space-y-2 p-2 bg-gray-50 rounded-md">
                                    ...
                                    <div>
                                        ...
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            <input
                                                :value="section.content.overlayGradientFrom || '#2563eb'"
                                                @input="updateContent('overlayGradientFrom', ($event.target as HTMLInputElement).value)"
                                                class="flex-1 px-1.5 py-1 border border-gray-300 rounded text-[10px] font-mono text-gray-900 bg-white"
                                            />
                                        </div>
                                    </div>
                                    <div>
                                ...
                                <!-- Gradient Presets -->
                                <div>
                                    <label class="block text-[10px] text-gray-500 mb-1">Presets</label>
                                    <div class="grid grid-cols-4 gap-1">
                                        <button
                                            ...
                                            class="h-5 rounded border border-gray-200 hover:scale-105 transition-transform"
                                ...
                                <div>
                                    <div class="flex items-center justify-between mb-0.5">
                                        <label class="text-[10px] text-gray-500">Balance</label>
                                        <span class="text-[10px] text-gray-500">{{ section.content.overlayGradientMidpoint || 50 }}%</span>
                                    </div>
                                    <input
                                        type="range"
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-0.5">
                                    ...
                                    <span class="text-[10px] text-gray-500">{{ section.content.overlayOpacity || 50 }}%</span>
                                </div>
                                <input
                                    type="range"
                ...
                <template v-else-if="section.type === 'page-header'">
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Subtitle (optional)</label>
                        <textarea
                            :value="section.content.subtitle"
                            @input="updateContent('subtitle', ($event.target as HTMLTextAreaElement).value)"
                            rows="2"
                    ...
                    <div class="border-t border-gray-200 pt-3">
                        <h4 class="text-xs font-medium text-gray-700 mb-2">Colors</h4>
                        <div class="space-y-2">
                            <div>
                                ...
                                <div class="flex items-center gap-1.5">
                                    ...
                                    <input
                                        :value="section.content.backgroundColor || '#1e40af'"
                                        @input="updateContent('backgroundColor', ($event.target as HTMLInputElement).value)"
                                        class="flex-1 px-1.5 py-1 border border-gray-300 rounded text-xs font-mono text-gray-900"
                                    />
                                </div>
                            </div>
                            <div>
                                <label class="block text-[10px] text-gray-500 mb-1">Text</label>
                                <div class="flex items-center gap-1.5">
                                    <input
                                        type="color"
                ...
                <template v-else-if="section.type === 'about'">
                    ...
                    <div>
                        ...
                        <div v-if="section.content.image" class="relative mb-1.5">
                            <img :src="section.content.image" class="w-full h-24 object-cover rounded-md" />
                            <button @click="updateContent('image', '')" class="absolute top-1 right-1 p-0.5 bg-red-500 text-white rounded-full hover:bg-red-600">
                                <XMarkIcon class="w-3 h-3" aria-hidden="true" />
                            </button>
                        </div>
                        <button
                            @click="emit('openMediaLibrary', section.id, 'image')"
                            class="w-full flex items-center justify-center gap-1.5 px-3 py-2 border-2 border-dashed border-gray-300 rounded-md text-gray-500 hover:border-blue-400 hover:text-blue-600 transition-colors"
                        >
                            <PhotoIcon class="w-4 h-4" aria-hidden="true" />
                            <span class="text-xs">{{ section.content.image ? 'Change' : 'Add Image' }}</span>
                        </button>
                    </div>
                    <div>
                ...
                <template v-else-if="section.type === 'services' || section.type === 'features'">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <label class="text-xs font-medium text-gray-700">Items</label>
                            <button @click="emit('addItem')" class="text-[10px] text-blue-600 hover:text-blue-700">+ Add</button>
                        </div>
                        <div class="space-y-2">
                            <div v-for="(item, idx) in section.content.items || []" :key="idx" class="p-2 bg-gray-50 rounded-md">
                                <div class="flex items-center justify-between mb-1.5">
                                    <span class="text-[10px] font-medium text-gray-500">Item {{ idx + 1 }}</span>
                                    <button @click="emit('removeItem', idx)" class="p-0.5 hover:bg-red-100 rounded">
                                        <TrashIcon class="w-3 h-3 text-red-500" aria-hidden="true" />
                                    </button>
                                </div>
                                
                                <!-- Icon Picker Button -->
                                <div class="mb-1.5">
                                    <label class="block text-[10px] text-gray-500 mb-0.5">Icon</label>
                                    <button
                                        @click="openIconPicker(idx)"
                                        class="w-full flex items-center gap-1.5 px-2 py-1.5 border border-gray-300 rounded-md hover:border-blue-400 hover:bg-blue-50 transition-colors"
                                    >
                                        <div class="w-6 h-6 bg-blue-100 rounded flex items-center justify-center">
                                            <Squares2X2Icon v-if="!item.icon" class="w-3 h-3 text-blue-600" aria-hidden="true" />
                                            <span v-else class="text-[9px] text-blue-600 font-medium">{{ item.icon?.replace('Icon', '').slice(0, 2) }}</span>
                                        </div>
                                        <span class="text-xs text-gray-600">{{ item.icon ? item.icon.replace('Icon', '') : 'Choose' }}</span>
                                    </button>
                                </div>
                                
                                <input v-model="item.title" placeholder="Title" class="w-full px-2 py-1 border border-gray-300 rounded text-xs mb-1.5 text-gray-900" />
                                <textarea v-model="item.description" placeholder="Description" rows="2" class="w-full px-2 py-1 border border-gray-300 rounded text-xs text-gray-900"></textarea>
                            </div>
                        </div>
                    </div>
                </template>
                ...
                <template v-else-if="section.type === 'testimonials'">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <label class="text-xs font-medium text-gray-700">Testimonials</label>
                            <button @click="emit('addItem')" class="text-[10px] text-blue-600 hover:text-blue-700">+ Add</button>
                        </div>
                        <div class="space-y-2">
                            <div v-for="(item, idx) in section.content.items || []" :key="idx" class="p-2 bg-gray-50 rounded-md">
                                <div class="flex items-center justify-between mb-1.5">
                                    <span class="text-[10px] font-medium text-gray-500">{{ idx + 1 }}</span>
                                    <button @click="emit('removeItem', idx)" class="p-0.5 hover:bg-red-100 rounded">
                                        <TrashIcon class="w-3 h-3 text-red-500" aria-hidden="true" />
                                    </button>
                                </div>
                                <input v-model="item.name" placeholder="Name" class="w-full px-2 py-1 border border-gray-300 rounded text-xs mb-1.5 text-gray-900" />
                                <input v-model="item.role" placeholder="Role" class="w-full px-2 py-1 border border-gray-300 rounded text-xs mb-1.5 text-gray-900" />
                                <textarea v-model="item.text" placeholder="Testimonial" rows="2" class="w-full px-2 py-1 border border-gray-300 rounded text-xs text-gray-900"></textarea>
                            </div>
                        </div>
                    </div>
                </template>
                ...
                <template v-else-if="section.type === 'pricing'">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <label class="text-xs font-medium text-gray-700">Plans</label>
                            <button @click="emit('addPlan')" class="text-[10px] text-blue-600 hover:text-blue-700">+ Add</button>
                        </div>
                        <div class="space-y-2">
                            <div v-for="(plan, idx) in section.content.plans || []" :key="idx" class="p-2 bg-gray-50 rounded-md">
                                <div class="flex items-center justify-between mb-1.5">
                                    <span class="text-[10px] font-medium text-gray-500">Plan {{ idx + 1 }}</span>
                                    <button @click="emit('removePlan', idx)" class="p-0.5 hover:bg-red-100 rounded">
                                        <TrashIcon class="w-3 h-3 text-red-500" aria-hidden="true" />
                                    </button>
                                </div>
                                <input v-model="plan.name" placeholder="Plan Name" class="w-full px-2 py-1 border border-gray-300 rounded text-xs mb-1.5 text-gray-900" />
                                <input v-model="plan.price" placeholder="Price (e.g., K99)" class="w-full px-2 py-1 border border-gray-300 rounded text-xs mb-1.5 text-gray-900" />
                                <div class="mt-1.5">
                                    <div class="flex items-center justify-between mb-0.5">
                                        <span class="text-[10px] text-gray-500">Features</span>
                                        <button @click="emit('addPlanFeature', idx)" class="text-[10px] text-blue-600">+ Add</button>
                                    </div>
                                    <div v-for="(feature, fIdx) in plan.features || []" :key="fIdx" class="flex items-center gap-1 mb-0.5">
                                        <input v-model="plan.features[fIdx]" class="flex-1 px-1.5 py-0.5 border border-gray-300 rounded text-[10px] text-gray-900" />
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <template v-else-if="section.type === 'gallery'">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <label class="text-xs font-medium text-gray-700">Images</label>
                            <button @click="emit('openMediaLibrary', section.id, 'images')" class="text-[10px] text-blue-600 hover:text-blue-700">+ Add</button>
                        </div>
                        <div class="grid grid-cols-3 gap-1.5">
                            <div v-for="(img, idx) in section.content.images || []" :key="img.id || idx" class="relative aspect-square">
                                ...
                                <button @click="emit('removeGalleryImage', idx)" class="absolute top-0.5 right-0.5 p-0.5 bg-red-500 text-white rounded-full hover:bg-red-600">
                                    <XMarkIcon class="w-2.5 h-2.5" aria-hidden="true" />
                                </button>
                            </div>
                        </div>
                        <div v-if="!section.content.images?.length" class="text-center py-4 text-gray-400 border-2 border-dashed border-gray-200 rounded-md">
                            <PhotoIcon class="w-6 h-6 mx-auto mb-1" aria-hidden="true" />
                            <p class="text-[10px]">No images yet</p>
                        </div>
                    </div>
                </template>
                ...
                <template v-else-if="section.type === 'member-cta'">
                    ...
                    <div class="border-t border-gray-200 pt-3">
                        <h4 class="text-xs font-medium text-gray-700 mb-2">Buttons</h4>
                        <div class="space-y-2">
                            <div>
                                <label class="block text-[10px] text-gray-500 mb-0.5">Register Button</label>
                                <input
                                    :value="section.content.registerText"
                                    @input="updateContent('registerText', ($event.target as HTMLInputElement).value)"
                                    class="w-full px-2 py-1 border border-gray-300 rounded text-xs text-gray-900"
                                    placeholder="Sign Up Now"
                                />
                            </div>
                            <div>
                                <label class="block text-[10px] text-gray-500 mb-0.5">Button Style</label>
                                <select
                            ...
                            <div class="flex items-center gap-1.5">
                                ...
                                />
                                <label for="showLoginLink" class="text-xs text-gray-700">Show login link</label>
                            </div>
                            <div v-if="section.content.showLoginLink !== false">
                                <label class="block text-[10px] text-gray-500 mb-0.5">Login Text</label>
                                <input
                ...
                <template v-else-if="section.type === 'text'">
                    <div>
                        ...
                        <RichTextEditor
                            ...
                            :darkMode="darkMode"
                        />
                        <p class="text-[10px] text-gray-500 mt-1">Ctrl+B/I/U for shortcuts</p>
                    </div>
                </template>
                ...
                <template v-else-if="section.type === 'faq'">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <label class="text-xs font-medium text-gray-700">Questions</label>
                            <button @click="emit('addFaqItem')" class="text-[10px] text-blue-600 hover:text-blue-700">+ Add</button>
                        </div>
                        <div class="space-y-2">
                            <div v-for="(item, idx) in section.content.items || []" :key="idx" class="p-2 bg-gray-50 rounded-md">
                                <div class="flex items-center justify-between mb-1.5">
                                    <span class="text-[10px] font-medium text-gray-500">Q{{ idx + 1 }}</span>
                                    <button @click="emit('removeFaqItem', idx)" class="p-0.5 hover:bg-red-100 rounded">
                                        <TrashIcon class="w-3 h-3 text-red-500" aria-hidden="true" />
                                    </button>
                                </div>
                                <input v-model="item.question" placeholder="Question" class="w-full px-2 py-1 border border-gray-300 rounded text-xs mb-1.5 text-gray-900" />
                                <textarea v-model="item.answer" placeholder="Answer" rows="2" class="w-full px-2 py-1 border border-gray-300 rounded text-xs text-gray-900"></textarea>
                            </div>
                        </div>
                    </div>
                </template>
                ...
                <template v-else-if="section.type === 'team'">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <label class="text-xs font-medium text-gray-700">Team Members</label>
                            <button @click="emit('addTeamMember')" class="text-[10px] text-blue-600 hover:text-blue-700">+ Add</button>
                        </div>
                        <div class="space-y-2">
                            <div v-for="(member, idx) in section.content.items || []" :key="idx" class="p-2 bg-gray-50 rounded-md">
                                <div class="flex items-center justify-between mb-1.5">
                                    <span class="text-[10px] font-medium text-gray-500">{{ idx + 1 }}</span>
                                    <button @click="emit('removeTeamMember', idx)" class="p-0.5 hover:bg-red-100 rounded">
                                        <TrashIcon class="w-3 h-3 text-red-500" aria-hidden="true" />
                                    </button>
                                </div>
                                <input v-model="member.name" placeholder="Name" class="w-full px-2 py-1 border border-gray-300 rounded text-xs mb-1.5 text-gray-900" />
                                <input v-model="member.role" placeholder="Role" class="w-full px-2 py-1 border border-gray-300 rounded text-xs mb-1.5 text-gray-900" />
                                <textarea v-model="member.bio" placeholder="Short bio" rows="2" class="w-full px-2 py-1 border border-gray-300 rounded text-xs text-gray-900"></textarea>
                            </div>
                        </div>
                    </div>
                </template>
                ...
                <template v-else-if="section.type === 'blog'">
                    <div class="p-2 bg-blue-50 border border-blue-200 rounded-md mb-3">
                        <p class="text-[10px] text-blue-700">
                            <strong>Dynamic:</strong> Posts from dashboard appear here.
                        </p>
                    </div>
                    ...
                    <div class="pt-2 border-t border-gray-200">
                        <label class="block text-xs font-medium text-gray-700 mb-1.5">Layout</label>
                        <div class="grid grid-cols-3 gap-1.5">
                            <button
                                @click="updateContent('layout', 'grid')"
                                :class="['p-1.5 border rounded-md text-[10px]', section.content.layout === 'grid' || !section.content.layout ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-300 text-gray-600']"
                            >Grid</button>
                            <button
                                @click="updateContent('layout', 'list')"
                                :class="['p-1.5 border rounded-md text-[10px]', section.content.layout === 'list' ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-300 text-gray-600']"
                            >List</button>
                            <button
                                @click="updateContent('layout', 'featured')"
                                :class="['p-1.5 border rounded-md text-[10px]', section.content.layout === 'featured' ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-300 text-gray-600']"
                            >Featured</button>
                        </div>
                    </div>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Posts to Show</label>
                        <select
                    ...
                    <div class="pt-2 border-t border-gray-200">
                        <label class="block text-xs font-medium text-gray-700 mb-1.5">Card Style</label>
                        <div class="grid grid-cols-2 gap-1.5">
                            <button
                                @click="updateContent('cardStyle', 'bordered')"
                                :class="['p-1.5 border rounded-md text-[10px]', section.content.cardStyle === 'bordered' || !section.content.cardStyle ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-300 text-gray-600']"
                            >Bordered</button>
                            <button
                                @click="updateContent('cardStyle', 'shadow')"
                                :class="['p-1.5 border rounded-md text-[10px]', section.content.cardStyle === 'shadow' ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-300 text-gray-600']"
                            >Shadow</button>
                            <button
                                @click="updateContent('cardStyle', 'minimal')"
                                :class="['p-1.5 border rounded-md text-[10px]', section.content.cardStyle === 'minimal' ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-300 text-gray-600']"
                            >Minimal</button>
                            <button
                                @click="updateContent('cardStyle', 'filled')"
                                :class="['p-1.5 border rounded-md text-[10px]', section.content.cardStyle === 'filled' ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-300 text-gray-600']"
                            >Filled</button>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <div>
                            <label class="block text-[10px] font-medium text-gray-700 mb-1">Card BG</label>
                            <input
                                type="color"
                                ...
                                class="w-full h-6 rounded border border-gray-300 cursor-pointer"
                            />
                        </div>
                        <div>
                            <label class="block text-[10px] font-medium text-gray-700 mb-1">Card Text</label>
                            <input
                                type="color"
                    ...
                    <div class="pt-2 border-t border-gray-200 space-y-1.5">
                        <label class="block text-xs font-medium text-gray-700 mb-1.5">Display</label>
                        <div class="flex items-center gap-1.5">
                            <input type="checkbox" :checked="section.content.showImage !== false" @change="updateContent('showImage', ($event.target as HTMLInputElement).checked)" id="blogShowImage" class="rounded border-gray-300 w-3.5 h-3.5" />
                            <label for="blogShowImage" class="text-xs text-gray-700">Image</label>
                        </div>
                        <div class="flex items-center gap-1.5">
                            <input type="checkbox" :checked="section.content.showDate !== false" @change="updateContent('showDate', ($event.target as HTMLInputElement).checked)" id="blogShowDate" class="rounded border-gray-300 w-3.5 h-3.5" />
                            <label for="blogShowDate" class="text-xs text-gray-700">Date</label>
                        </div>
                        <div class="flex items-center gap-1.5">
                            <input type="checkbox" :checked="section.content.showExcerpt !== false" @change="updateContent('showExcerpt', ($event.target as HTMLInputElement).checked)" id="blogShowExcerpt" class="rounded border-gray-300 w-3.5 h-3.5" />
                            <label for="blogShowExcerpt" class="text-xs text-gray-700">Excerpt</label>
                        </div>
                        <div class="flex items-center gap-1.5">
                            <input type="checkbox" :checked="section.content.showViewAll !== false" @change="updateContent('showViewAll', ($event.target as HTMLInputElement).checked)" id="blogShowViewAll" class="rounded border-gray-300 w-3.5 h-3.5" />
                            <label for="blogShowViewAll" class="text-xs text-gray-700">View All button</label>
                        </div>
                    </div>
                    ...
                    <div class="mt-3 p-2 bg-gray-50 border border-gray-200 rounded-md">
                        <a
                            ...
                            target="_blank"
                            class="inline-flex items-center gap-1 text-[10px] text-blue-600 hover:text-blue-700"
                        >
                            Manage Posts →
                        </a>
                    </div>
                ...
                <template v-else-if="section.type === 'stats'">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <label class="text-xs font-medium text-gray-700">Stats</label>
                            <button @click="emit('addStatItem')" class="text-[10px] text-blue-600 hover:text-blue-700">+ Add</button>
                        </div>
                        <div class="space-y-2">
                            <div v-for="(stat, idx) in section.content.items || []" :key="idx" class="p-2 bg-gray-50 rounded-md">
                                <div class="flex items-center justify-between mb-1.5">
                                    <span class="text-[10px] font-medium text-gray-500">{{ idx + 1 }}</span>
                                    <button @click="emit('removeStatItem', idx)" class="p-0.5 hover:bg-red-100 rounded">
                                        <TrashIcon class="w-3 h-3 text-red-500" aria-hidden="true" />
                                    </button>
                                </div>
                                <input v-model="stat.value" placeholder="Value (e.g., 500+)" class="w-full px-2 py-1 border border-gray-300 rounded text-xs mb-1.5 text-gray-900" />
                                <input v-model="stat.label" placeholder="Label" class="w-full px-2 py-1 border border-gray-300 rounded text-xs text-gray-900" />
                            </div>
                        </div>
                    </div>
                </template>
                ...
                <template v-else-if="section.type === 'map'">
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Maps Embed URL</label>
                        <input
                        ...
                        />
                        <p class="text-[10px] text-gray-500 mt-0.5">Google Maps → Share → Embed</p>
                    </div>
                    <div>
                    ...
                    <div class="flex items-center gap-1.5">
                        ...
                        />
                        <label class="text-xs text-gray-700">Show address below map</label>
                    </div>
                </template>
                ...
                <template v-else-if="section.type === 'video'">
                    ...
                    <div>
                        ...
                        />
                        <p class="text-[10px] text-gray-500 mt-0.5">e.g., https://www.youtube.com/embed/VIDEO_ID</p>
                    </div>
                </template>
                ...
                <template v-else-if="section.type === 'divider'">
                    ...
                    <div v-if="section.content.style !== 'space'">
                        ...
                        <input
                            ...
                            class="w-full h-8 rounded-md cursor-pointer"
                        />
                    </div>
                </template>
            </div>
            ...
            <div v-if="activeTab === 'style'" class="space-y-3">
                <!-- Section Height -->
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1.5">Section Height</label>
                    <div class="flex items-center gap-2">
                        ...
                        <div class="flex items-center gap-0.5">
                            <input
                                ...
                                min="100"
                                max="800"
                                class="w-14 px-1.5 py-1 border border-gray-300 rounded text-xs text-center text-gray-900"
                            />
                ...
                <!-- Content Alignment -->
                <div class="border-t border-gray-200 pt-3">
                    <label class="block text-xs font-medium text-gray-700 mb-1.5">Alignment</label>
                    <div class="space-y-2">
                        <!-- Horizontal Alignment -->
                        <div>
                            <label class="block text-[10px] text-gray-500 mb-1">Horizontal</label>
                            <div class="flex gap-1.5">
                                <button
                                    @click="updateContent('textPosition', 'left'); updateStyle('textAlign', 'left')"
                                    :class="[
                                        'flex-1 flex items-center justify-center gap-1 px-2 py-1.5 rounded-md border-2 text-xs font-medium transition-colors',
                        ...
                        <div v-if="['services', 'features', 'testimonials', 'team', 'stats', 'pricing', 'gallery'].includes(section.type)">
                            <label class="block text-[10px] text-gray-500 mb-1">Items Layout</label>
                            <div class="flex gap-1.5">
                                <button
                                    @click="updateStyle('itemsAlign', 'start')"
                                    :class="[
                                        'flex-1 px-2 py-1.5 rounded-md border-2 text-[10px] font-medium transition-colors',
                ...
                <template v-if="!section.style?.backgroundType || section.style?.backgroundType === 'solid'">
                    <div>
                        ...
                        <div class="grid grid-cols-4 gap-1.5">
                            <button
                                ...
                                :class="['w-full aspect-square rounded-md border-2 transition-all', section.style?.backgroundColor === color.value ? 'border-blue-500 ring-2 ring-blue-200' : 'border-gray-200 hover:border-gray-300']"
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Custom</label>
                        <input
                            type="color"
                ...
                <template v-else-if="section.style?.backgroundType === 'gradient'">
                    <!-- Gradient Presets -->
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1.5">Presets</label>
                        <div class="grid grid-cols-4 gap-1.5">
                            <button
                                v-for="(preset, idx) in gradientPresets"
                                :key="idx"
                                @click="applyGradientPreset(preset)"
                                class="w-full aspect-square rounded-md border-2 border-gray-200 hover:border-blue-400 transition-all"
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-4 gap-1.5">
                            <button
                                ...
                                :class="[
                                    'px-2 py-1.5 text-xs font-medium rounded-md border-2 transition-colors',
                    ...
                    <!-- Gradient Preview -->
                    <div>
                        <label class="block text-[10px] text-gray-500 mb-1">Preview</label>
                        <div 
                            class="h-8 rounded-md border border-gray-200"
            ...
            <div v-if="activeTab === 'advanced'" class="space-y-3">
                <div class="p-2 bg-gray-50 rounded-md">
                    <p class="text-xs text-gray-500">ID: <code class="text-[10px] bg-gray-200 px-1 rounded">{{ section.id }}</code></p>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">CSS Class</label>
                    <input
                        :value="section.style?.cssClass || ''"
                        @input="updateStyle('cssClass', ($event.target as HTMLInputElement).value)"
                        class="w-full px-2 py-1.5 border border-gray-300 rounded-md text-xs font-mono text-gray-900"
