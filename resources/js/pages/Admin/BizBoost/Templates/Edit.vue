...
const form = useForm({
    ...
    template_data: {
        caption: props.template.template_data?.caption || '',
        hashtags: props.template.template_data?.hashtags || [],
        cta: props.template.template_data?.cta || '',
    },
...
const previewUrl = ref<string | null>(
    props.template.preview_path ? `/storage/${props.template.preview_path}` : null
);
...
const submit = () => {
    form.post(`/admin/bizboost/templates/${props.template.id}`, {
        forceFormData: true,
        _method: 'PUT',
    });
};
