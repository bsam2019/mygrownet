...
<template>
  <AdminLayout :breadcrumbs="[
    ...
    <div v-if="showMobileActionsModal && selectedUser" class="fixed inset-0 z-50 flex items-end md:items-center md:justify-center">
      <!-- Background Overlay -->
      <div class="absolute inset-0 bg-gray-500 bg-opacity-75" @click="showMobileActionsModal = false"></div>
      
      <!-- Modal Content -->
      <div class="relative w-full md:max-w-md bg-white rounded-t-2xl md:rounded-2xl shadow-xl p-4 max-h-[80vh] overflow-y-auto">
          <!-- Header -->
          ...
          <div class="space-y-2">
            ...
            <button
              ...
              class="w-full px-4 py-3 text-left text-sm font-medium text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 flex items-center justify-between group relative"
              title="Quick advance with automatic repayment (no interest)"
            >
              <span>Issue Quick Advance</span>
            ...
            <Link
              :href="route('admin.platform-loans.create', { user_id: selectedUser.id })"
              @click="showMobileActionsModal = false"
              class="w-full px-4 py-3 text-left text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 flex items-center justify-between"
              title="Formal loan with interest, terms, and schedules"
            >
              <span>Issue Formal Loan</span>
              <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <button
              @click="openLoanLimitModal(selectedUser); showMobileActionsModal = false"
              class="w-full px-4 py-3 text-left text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 flex items-center justify-between"
            >
              <span>Set Loan Limit</span>
              <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
