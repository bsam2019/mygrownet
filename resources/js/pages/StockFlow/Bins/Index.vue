...
const uploadBinsCsv = async (event: Event) => {
    const input = event.target as HTMLInputElement;
    if (!input.files?.length) return;
    const formData = new FormData();
    formData.append('file', input.files[0]);
    await router.post(route('stockflow.sub.bins.import-csv'), formData, {
        onSuccess: () => { success('Bins imported'); input.value = ''; },
            ...
            <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
                <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex gap-2">
                        <a :href="route('stockflow.sub.templates.bins')" class="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-600 hover:text-emerald-600 hover:border-emerald-200">
                        ...
                        <label class="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-600 hover:text-emerald-600 hover:border-emerald-200 cursor-pointer">
                            ...
                            Import CSV
                            <input type="file" accept=".csv,.txt" class="hidden" @change="uploadBinsCsv" />
                        </label>
