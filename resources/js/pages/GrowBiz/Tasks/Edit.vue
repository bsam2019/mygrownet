    ...
    <GrowBizLayout>
        <div class="px-4 py-4 pb-6">
            ...
            <div class="flex items-center gap-3 mb-6">
                <Link 
                    :href="route('growbiz.tasks.show', task.id)" 
            ...
            <div class="max-w-2xl mx-auto">
                <form @submit.prevent="submit" class="bg-white rounded-2xl shadow-sm p-5">
                    ...
                    <div class="mt-8 flex flex-col sm:flex-row items-center justify-end gap-3">
                        <Link :href="route('growbiz.tasks.show', task.id)" class="w-full sm:w-auto px-6 py-3 text-center text-gray-600 bg-gray-100 rounded-xl font-medium hover:bg-gray-200 active:bg-gray-300 transition-colors">
                            Cancel
                        </Link>
...
interface Props {
    task: { id: number; title: string; description: string | null; status: string; priority: string; due_date: string | null; };
    employees: Employee[];
...
const submit = () => {
    form.put(route('growbiz.tasks.update', props.task.id), {
        onSuccess: () => {
