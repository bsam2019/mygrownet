                ...
                <div class="flex items-center justify-between px-4 py-2 bg-gray-800 border-b border-gray-700">
                    <div class="flex items-center gap-4">
                        ...
                        <div class="flex items-center gap-1 text-xs text-gray-400">
                            ...
                            <span class="text-purple-400">{{ outputDimensions.width }}×{{ outputDimensions.height }}</span>
                ...
                <div class="flex-1 flex overflow-hidden">
                    ...
                    <div class="flex-1 flex items-center justify-center p-4 bg-gray-950 overflow-hidden">
                        ...
                        <div v-else ref="containerRef" class="relative max-w-full max-h-full" style="display: inline-block;">
                            ...
                            <!-- Crop Overlay -->
                            <div
                                class="absolute cursor-move border-2 border-purple-500 shadow-lg"
                                :style="{
                            ...
                            >
                                <!-- Corner Handles -->
                                <div class="absolute -top-1.5 -left-1.5 w-3 h-3 bg-purple-500 border-2 border-white cursor-nw-resize rounded-sm" @mousedown="startResize($event, 'nw')"></div>
                                <div class="absolute -top-1.5 -right-1.5 w-3 h-3 bg-purple-500 border-2 border-white cursor-ne-resize rounded-sm" @mousedown="startResize($event, 'ne')"></div>
                                <div class="absolute -bottom-1.5 -left-1.5 w-3 h-3 bg-purple-500 border-2 border-white cursor-sw-resize rounded-sm" @mousedown="startResize($event, 'sw')"></div>
                                <div class="absolute -bottom-1.5 -right-1.5 w-3 h-3 bg-purple-500 border-2 border-white cursor-se-resize rounded-sm" @mousedown="startResize($event, 'se')"></div>
                                <!-- Edge Handles -->
                                <div class="absolute top-1/2 -left-1.5 w-3 h-6 -translate-y-1/2 bg-purple-500 border-2 border-white cursor-w-resize rounded-sm" @mousedown="startResize($event, 'w')"></div>
                                <div class="absolute top-1/2 -right-1.5 w-3 h-6 -translate-y-1/2 bg-purple-500 border-2 border-white cursor-e-resize rounded-sm" @mousedown="startResize($event, 'e')"></div>
                                <div class="absolute -top-1.5 left-1/2 w-6 h-3 -translate-x-1/2 bg-purple-500 border-2 border-white cursor-n-resize rounded-sm" @mousedown="startResize($event, 'n')"></div>
                                <div class="absolute -bottom-1.5 left-1/2 w-6 h-3 -translate-x-1/2 bg-purple-500 border-2 border-white cursor-s-resize rounded-sm" @mousedown="startResize($event, 's')"></div>
                                <!-- Grid -->
                    ...
                    <div class="w-64 bg-gray-800 border-l border-gray-700 flex flex-col">
                        ...
                        <div class="flex border-b border-gray-700">
                            <button @click="activeTab = 'crop'" :class="['flex-1 py-2 text-xs font-medium transition-colors', activeTab === 'crop' ? 'text-purple-400 border-b-2 border-purple-400 bg-gray-700/50' : 'text-gray-400 hover:text-white']">Crop</button>
                            <button @click="activeTab = 'adjust'" :class="['flex-1 py-2 text-xs font-medium transition-colors', activeTab === 'adjust' ? 'text-purple-400 border-b-2 border-purple-400 bg-gray-700/50' : 'text-gray-400 hover:text-white']">Adjust</button>
                            <button @click="activeTab = 'export'" :class="['flex-1 py-2 text-xs font-medium transition-colors', activeTab === 'export' ? 'text-purple-400 border-b-2 border-purple-400 bg-gray-700/50' : 'text-gray-400 hover:text-white']">Export</button>
                        </div>
                        ...
                        <div class="flex-1 overflow-y-auto p-3 space-y-4">
                                ...
                                <div>
                                    ...
                                    <div class="grid grid-cols-3 gap-1">
                                        <button v-for="r in aspectRatios" :key="r.label" @click="applyAspectRatio(r.value)"
                                            :class="['px-2 py-1.5 text-xs rounded transition-colors', selectedRatio === r.value ? 'bg-purple-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600']">
                                            {{ r.label }}
                                        </button>
                                    </div>
                                </div>
                            ...
                            <template v-if="activeTab === 'adjust'">
                                <div>
                                    ...
                                    <input type="range" v-model.number="adjustments.brightness" min="0" max="200" class="w-full h-1.5 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-purple-500" />
                                </div>
                                <div>
                                    <div class="flex justify-between text-xs mb-1">
                                    ...
                                    <input type="range" v-model.number="adjustments.contrast" min="0" max="200" class="w-full h-1.5 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-purple-500" />
                                </div>
                                <div>
                                    <div class="flex justify-between text-xs mb-1">
                                    ...
                                    <input type="range" v-model.number="adjustments.saturation" min="0" max="200" class="w-full h-1.5 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-purple-500" />
                                </div>
                                <div>
                                    <div class="flex justify-between text-xs mb-1">
                                    ...
                                    <input type="range" v-model.number="adjustments.blur" min="0" max="20" step="0.5" class="w-full h-1.5 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-purple-500" />
                                </div>
                            ...
                            <template v-if="activeTab === 'export'">
                                <div>
                                    ...
                                    <div class="grid grid-cols-3 gap-1">
                                        <button v-for="fmt in ['jpeg', 'png', 'webp']" :key="fmt" @click="exportFormat = fmt as any"
                                            :class="['px-2 py-1.5 text-xs rounded uppercase transition-colors', exportFormat === fmt ? 'bg-purple-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600']">
                                            {{ fmt }}
                                        </button>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <input type="range" v-model.number="exportQuality" min="10" max="100" class="w-full h-1.5 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-purple-500" />
                                </div>
                                <div>
                                    <div class="flex justify-between text-xs mb-1">
                                    ...
                                    <input type="range" v-model.number="exportScale" min="10" max="200" class="w-full h-1.5 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-purple-500" />
                        ...
                        <div class="p-3 border-t border-gray-700">
                            <button @click="saveCrop" class="w-full flex items-center justify-center gap-2 py-2.5 bg-purple-600 text-white text-sm font-medium rounded-lg hover:bg-purple-700 transition-colors">
