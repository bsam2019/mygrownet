...
import WeddingImageEditorModal from './WeddingImageEditorModal.vue';
...
const stockCategories = ['wedding', 'couple', 'love', 'romantic', 'flowers', 'celebration', 'nature', 'elegant'];
...
const searchStockPhotos = async (query: string) => {
    ...
    try {
        ...
        stockPhotos.value = Array.from({ length: 16 }, (_, i) => {
            const imageId = (baseId + i * 10) % 1000 + 100;
            return {
                id: `stock-${query}-${i}`,
                urls: {
            ...
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[85vh] flex flex-col" @click.stop>
                ...
                <div class="border-b border-gray-200">
                    ...
                    <div class="flex gap-1 px-4 mt-3">
                        ...
                        <button
                            @click="activeTab = 'stock'; stockPhotos.length === 0 && searchStockPhotos('wedding')"
                            :class="[
                                'flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-t-lg border-b-2 transition-colors',
                                activeTab === 'stock'
                                    ? 'text-purple-600 border-purple-600 bg-purple-50'
                ...
                <template v-if="activeTab === 'library'">
                    ...
                    <div class="p-4 border-b border-gray-100 bg-gray-50">
                        <label class="flex flex-col items-center justify-center w-full h-20 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-purple-400 hover:bg-purple-50/50 transition-colors">
                            <div class="flex items-center gap-3">
                                <CloudArrowUpIcon class="w-6 h-6 text-gray-400" />
                                <div class="text-left">
                                    <p class="text-sm text-gray-600">
                                        <span v-if="uploading">Uploading...</span>
                                        <span v-else><span class="text-purple-600 font-medium">Click to upload</span> or drag and drop</span>
                                    </p>
                    ...
                    <div class="flex-1 overflow-y-auto p-4">
                        <div v-if="mediaLibrary.length > 0" class="grid grid-cols-4 md:grid-cols-5 gap-3">
                            <div
                                ...
                                :key="media.id"
                                class="group relative aspect-square rounded-lg overflow-hidden border-2 border-transparent hover:border-purple-500 transition-colors cursor-pointer bg-gray-100"
                            ...
                            >
                                ...
                                <div class="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
                                    ...
                                    <button @click="handleDirectSelect(media, $event)" class="p-2 bg-purple-600 rounded-full shadow-lg hover:bg-purple-700" title="Select">
                ...
                <template v-if="activeTab === 'stock'">
                    ...
                    <div class="p-4 border-b border-gray-100 bg-gray-50">
                        <div class="flex gap-2">
                            ...
                            <button @click="handleStockSearch" :disabled="stockLoading" class="px-4 py-2 bg-purple-600 text-white font-medium rounded-lg hover:bg-purple-700 disabled:opacity-50">
                                Search
                            </button>
                        </div>
                        ...
                        <div class="flex flex-wrap gap-2 mt-3">
                            <button
                                ...
                                :class="[
                                    ...
                                    stockSearchQuery === cat ? 'bg-purple-600 text-white' : 'bg-white border border-gray-300 text-gray-600 hover:bg-gray-100'
                                ]"
                            >
                                {{ cat }}
                            </button>
                    ...
                    <div class="flex-1 overflow-y-auto p-4">
                        ...
                        <div v-else-if="stockPhotos.length > 0" class="grid grid-cols-3 md:grid-cols-4 gap-3">
                            <div
                                ...
                                class="group relative aspect-[4/3] rounded-lg overflow-hidden bg-gray-100 hover:ring-2 hover:ring-purple-500 transition-all cursor-pointer"
                            >
                                ...
                                <div class="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
                                    ...
                                    <button @click="selectStockPhotoDirect(photo, $event)" class="p-2 bg-purple-600 rounded-full shadow-lg hover:bg-purple-700" title="Use directly">
                    ...
                    <div class="px-4 py-2 border-t border-gray-200 bg-gray-50 text-center">
                        <p class="text-xs text-gray-500">
                            Demo photos by <a href="https://picsum.photos" target="_blank" class="text-purple-600 hover:underline">Lorem Picsum</a> • Free to use
                        </p>
                    </div>
                </template>
            </div>
