...
function switchToOrganization(org: any) {
    ctxOpen.value = false;
    router.post(route('workspace.switch-context'), { type: 'organization', organization_id: org.id });
}
function handleClickOutside(e: MouseEvent) {
    if (ctxMenuRef.value && !ctxMenuRef.value.contains(e.target as Node)) ctxOpen.value = false;
}
...
function handleUserClickOutside(e: MouseEvent) {
    if (userMenuRef.value && !userMenuRef.value.contains(e.target as Node)) userOpen.value = false;
}
    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-950 flex transition-colors duration-200">
        ...
        >
            ...
            <nav class="flex-1 flex flex-col py-4 overflow-y-auto overflow-x-hidden" :class="showCollapsed ? 'items-center px-1 space-y-1' : 'px-3 space-y-0.5'">
                <div v-if="!showCollapsed" class="px-3 pb-3 text-[10px] font-semibold text-gray-400 dark:text-white/25 uppercase tracking-[0.15em]">{{ title || 'Navigation' }}</div>
                ...
                >
                    <div class="relative flex-shrink-0">
                        ...
                        <span
                            v-if="item.badge"
                            class="absolute -top-1.5 -right-1.5 flex items-center justify-center w-4 h-4 rounded-full bg-red-500 text-[8px] font-bold text-white"
                        >{{ item.badge }}</span>
                    </div>
            ...
            <!-- Bottom section (theme, context, user) -->
